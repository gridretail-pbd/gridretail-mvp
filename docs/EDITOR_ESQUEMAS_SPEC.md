...
### 3.4 Modal: Agregar/Editar Partida (SchemeItemModal)

**Propósito:** Configurar una partida usando presets o de forma personalizada

**Diseño UI v2.1:** Interface simplificada con 3 tabs iguales
...
│  └───────────────────┴───────────────────┴───────────────────┘             │
│                                                                             │
│  [Tab: Agrupaciones] - Grid 2 columnas                                      │
│  ┌─────────────────┐ ┌─────────────────┐                                   │
│  │ OSS          ✓  │ │ OPP             │                                   │
│  └─────────────────┘ └─────────────────┘                                   │
│  ┌─────────────────┐ ┌─────────────────┐                                   │
...
│  └─────────────────┘ └─────────────────┘                                   │
│                                                                             │
│  [Tab: Individuales] - Grid 4 columnas                                      │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐                       │
│  │ OSS BASE │ │OSS CAPT. │ │ OPP BASE │ │OPP CAPT. │                       │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘                       │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐                       │
│  │ VR BASE  │ │VR CAPTURA│ │ VR MONO  │ │ MISS IN  │   ... más             │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘                       │
│                                                                             │
│  [Tab: Personalizado]                                                       │
...
│  [______OSS______________________________]                                  │
│                                                                             │
│  ─────────────────────────────────────────────────────────────────────────  │
...
│  │ [ACCESORIOS] [SEGURO]                                               │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  3 tipo(s) seleccionado(s)                                                 │
│                                                                             │
│  ─────────────────────────────────────────────────────────────────────────  │
│  Configuración de Meta                                                      │
│  Meta (unidades): [___31.5___]    Peso (% del variable): [___45___]        │
│  Factor Mix: [___0.27___]                                                  │
│                                                                             │
│  ─────────────────────────────────────────────────────────────────────────  │
│  Comisión                                                                   │
│  Variable máximo (S/.): [___324.00___]                                     │
...
**Comportamiento:**
- Al seleccionar un preset (Agrupación o Individual), se auto-completa:
  - `custom_name` con el nombre del preset
  - `tipos_venta_ids` con los tipos de venta pre-configurados
- Al cambiar de tab, se limpia la selección previa
- Los botones de preset muestran solo el nombre y ✓ cuando está seleccionado (sin descripciones)
- Los tipos de venta se muestran agrupados por categoría en orden: **POSTPAGO → PREPAGO → RENO → PACK SS → PACK → OTROS**

**Presets de Agrupaciones (desde BD `partition_presets` con `preset_group='agrupacion'`):**

| Code | Nombre Display | Tipos de venta incluidos |
|------|----------------|--------------------------|
| AGR_OSS | OSS | OSS_BASE, OSS_CAPTURA, PACK_OSS (línea) |
| AGR_OPP | OPP | OPP_BASE, OPP_CAPTURA |
| AGR_VR_CAPTURA | VR CAPTURA | VR_MONO, VR_CAPTURA, PACK_VR (línea) |
| AGR_VR_BASE | VR BASE | VR_BASE, PACK_VR_BASE (línea) |
| AGR_PACK_SS | PACK SS | PACK_OSS (equipo), PACK_VR (equipo) |
| AGR_RENO | RENO SS | RENO |
| AGR_PREPAGO | PREPAGO | PREPAGO, PORTA_PP |
| AGR_MISS_IN | MISS IN | MISS_IN |
| AGR_ACCESORIOS | ACCESORIOS | ACCESORIOS |

**Presets Individuales (desde BD `partition_presets` con `preset_group='individual'`):**

Cada tipo de venta tiene su preset individual con código `IND_{codigo}`:
- IND_OSS_BASE, IND_OSS_CAPTURA, IND_OPP_BASE, IND_OPP_CAPTURA
- IND_VR_BASE, IND_VR_CAPTURA, IND_VR_MONO, IND_MISS_IN
- IND_PACK_OSS, IND_PACK_VR, IND_PACK_OPEN, etc.
...
**Uso:** Mapeo flexible para mostrar qué ventas alimentan cada partida.

```typescript
// Obtener todos los tipos de venta activos
const { data: tiposVenta } = await supabase
  .from('tipos_venta')
  .select('id, codigo, nombre, categoria, requiere_iccid, requiere_imei')
  .eq('is_active', true)
  .order('categoria, codigo');
```

### 6.4 Tablas v2.1: Sistema de Presets

**Tabla `partition_presets`:** Define los presets disponibles para crear partidas.

```typescript
// Obtener presets con sus tipos de venta pre-configurados
  ...
  .select(`
    *,
    partition_preset_ventas (
      ...
      cuenta_linea,
      cuenta_equipo,
      tipos_venta (id, codigo, nombre, categoria)
    )
  `)
  .eq('is_active', true)
  .order('preset_group, display_order');
```

**Tabla `partition_preset_ventas`:** Mapea qué tipos de venta incluye cada preset.

| Columna | Descripción |
|---------|-------------|
| preset_id | FK a partition_presets |
| tipo_venta_id | FK a tipos_venta |
| cuenta_linea | Si cuenta como línea para esta partida |
| cuenta_equipo | Si cuenta como equipo para esta partida |
| display_order | Orden de visualización |

**Tabla `commission_item_ventas`:** Mapea qué tipos de venta aplican a cada partida de un esquema específico.

```typescript
// Al crear una partida, guardar los tipos de venta seleccionados
...
await supabase.from('commission_item_ventas').insert(ventasData);
```
...
|----------|--------|-------------|
| `/api/comisiones/presets` | GET | Obtiene presets con tipos de venta mapeados |
| `/api/comisiones/tipos-venta` | GET | Obtiene todos los tipos de venta activos |

### 6.5 Para el Simulador (futuro)
...
interface CommissionSchemeItem {
  id: string;
  scheme_id: string;
  // v2.1: Ahora item_type_id es opcional (puede usar preset o ser custom)
  item_type_id?: string;
  item_type?: CommissionItemType; // joined
  // v2.1: Referencia al preset usado
  preset_id?: string;
  preset?: PartitionPreset; // joined
  // v2.1: Nombre y descripción personalizados
  custom_name?: string;
  custom_description?: string;
  // v2.1: Tipos de venta mapeados (de commission_item_ventas)
  tipos_venta?: Array<{
...
// v2.1: Nuevo tipo para presets
interface PartitionPreset {
  id: string;
  code: string;
  name: string;
  short_name: string;
  ...
  preset_group: 'agrupacion' | 'individual';
  display_order: number;
  is_active: boolean;
}
...
// v2.1: Schema para selección de tipo de venta con flags
...
// v2.1: Schema actualizado con sistema de presets
export const schemeItemFormSchema = z.object({
  ...
  mix_factor: z.number().optional().nullable(),
  variable_amount: z.number().min(0),
  min_fulfillment: z.number().min(0).max(1).optional().nullable(),
  has_cap: z.boolean(),
  cap_percentage: z.number().min(0).max(2).optional().nullable(),
