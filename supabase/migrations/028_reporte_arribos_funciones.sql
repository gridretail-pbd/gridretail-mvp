-- ============================================================
-- MIGRACIÓN: Reporte de Arribos v2.0
-- Fecha: 2026-06-09
-- Descripción: Funciones SQL para el módulo de reportes de arribos
-- ============================================================
...
-- Índice para consultas por fecha y hora
CREATE INDEX IF NOT EXISTS idx_arribos_fecha_hora 
ON arribos(fecha, (EXTRACT(HOUR FROM hora)));

-- Índice para filtro por tienda y fecha
CREATE INDEX IF NOT EXISTS idx_arribos_tienda_fecha 
ON arribos(tienda_id, fecha);

-- Índice para cálculo de conversión
CREATE INDEX IF NOT EXISTS idx_arribos_conversion 
ON arribos(tienda_id, fecha, tipo_visita, se_vendio);

-- Índice para segmentación
CREATE INDEX IF NOT EXISTS idx_arribos_segmentacion
ON arribos(tienda_id, fecha, es_cliente_entel, tipo_documento_cliente);

-- ============================================================
-- 2. FUNCIÓN: Matriz de arribos por hora (vista principal)
-- ============================================================

CREATE OR REPLACE FUNCTION get_arribos_matriz(
  p_fecha DATE,
...
BEGIN
  ...
  pivoteado AS (
    SELECT 
      tienda_id,
      COALESCE(SUM(cantidad) FILTER (WHERE hora_slot = 8), 0)::INTEGER AS hora_8,
      COALESCE(SUM(cantidad) FILTER (WHERE hora_slot = 9), 0)::INTEGER AS hora_9,
      COALESCE(SUM(cantidad) FILTER (WHERE hora_slot = 10), 0)::INTEGER AS hora_10,
      COALESCE(SUM(cantidad) FILTER (WHERE hora_slot = 11), 0)::INTEGER AS hora_11,
      COALESCE(SUM(cantidad) FILTER (WHERE hora_slot = 12), 0)::INTEGER AS hora_12,
      COALESCE(SUM(cantidad) FILTER (WHERE hora_slot = 13), 0)::INTEGER AS hora_13,
      COALESCE(SUM(cantidad) FILTER (WHERE hora_slot = 14), 0)::INTEGER AS hora_14,
      COALESCE(SUM(cantidad) FILTER (WHERE hora_slot = 15), 0)::INTEGER AS hora_15,
      COALESCE(SUM(cantidad) FILTER (WHERE hora_slot = 16), 0)::INTEGER AS hora_16,
      COALESCE(SUM(cantidad) FILTER (WHERE hora_slot = 17), 0)::INTEGER AS hora_17,
      COALESCE(SUM(cantidad) FILTER (WHERE hora_slot = 18), 0)::INTEGER AS hora_18,
      COALESCE(SUM(cantidad) FILTER (WHERE hora_slot = 19), 0)::INTEGER AS hora_19,
      COALESCE(SUM(cantidad) FILTER (WHERE hora_slot = 20), 0)::INTEGER AS hora_20,
      COALESCE(SUM(cantidad) FILTER (WHERE hora_slot = 21), 0)::INTEGER AS hora_21
    FROM arribos_por_hora
    GROUP BY tienda_id
  )
  SELECT 
    t.id AS tienda_id,
    ...
    p.hora_20,
    p.hora_21,
    (p.hora_8 + p.hora_9 + p.hora_10 + p.hora_11 + p.hora_12 + p.hora_13 + 
     p.hora_14 + p.hora_15 + p.hora_16 + p.hora_17 + p.hora_18 + p.hora_19 + 
     p.hora_20 + p.hora_21)::INTEGER AS total,
    -- Calcular hora pico
    (SELECT hora_slot FROM arribos_por_hora aph 
     WHERE aph.tienda_id = t.id 
     ORDER BY cantidad DESC LIMIT 1) AS hora_pico
  FROM tiendas t
  JOIN pivoteado p ON t.id = p.tienda_id
  WHERE t.activa = true
    AND (p_zona IS NULL OR t.zona = p_zona)
  ORDER BY (p.hora_8 + p.hora_9 + p.hora_10 + p.hora_11 + p.hora_12 + p.hora_13 + 
            p.hora_14 + p.hora_15 + p.hora_16 + p.hora_17 + p.hora_18 + p.hora_19 + 
            p.hora_20 + p.hora_21) DESC;
END;
$$;

COMMENT ON FUNCTION get_arribos_matriz IS 
'Retorna matriz de arribos por tienda y hora para la vista principal del reporte';

-- ============================================================
-- 3. FUNCIÓN: Métricas por tienda (con comparación)
-- ============================================================
  ...
  IF p_comparacion IN ('N-1', 'N-7') THEN
    ...
    WITH metricas_hoy AS (
      SELECT 
        ...
        COUNT(*) FILTER (WHERE tipo_visita = 'VENTA' AND se_vendio = true)::INTEGER AS ventas,
...
'Retorna métricas de arribos por tienda con comparación temporal para el tab Métricas';

-- ============================================================
-- 4. FUNCIÓN: Resumen de red (KPIs header)
-- ============================================================
...
'Retorna KPIs de resumen de toda la red para el header del reporte';

-- ============================================================
-- 5. FUNCIÓN: Detalle de tienda (para sidebar y página expandida)
-- ============================================================
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      ...
      ELSE ROUND((SELECT COUNT(*) FROM hoy WHERE tipo_visita = 'VENTA' AND se_vendio = true)::NUMERIC / 
    ...
    (SELECT COUNT(*)::INTEGER FROM hoy WHERE tipo_visita = 'VENTA' AND se_vendio = true) AS vendio,
    (SELECT COUNT(*)::INTEGER FROM hoy WHERE tipo_visita = 'VENTA' AND se_vendio = false) AS no_vendio,
    (SELECT COUNT(*)::INTEGER FROM hoy WHERE tipo_visita = 'POSVENTA' AND se_vendio = true) AS crosssell_posventa,
    -- Segmentación
...
'Retorna detalle completo de una tienda para sidebar y página expandida';
...
GRANT EXECUTE ON FUNCTION get_arribos_matriz TO authenticated;
